...
function CfgInputTFunction({ paramDef, value, setValue }) {
  if (!value) {
    return <Box style={{ paddingLeft: '.5rem' }}><FunctionPicker add={(type) => setValue({ type, params: {} })} /></Box>
  } else {
    const def = definitions.tfunctionIndex[value.type]

    return (
      <Flex direction="row" align="flex-start" marginBottom="1rem" style={{ paddingLeft: '.5rem' }}>
        <Box2 flex="1 0 0">
          <Label p="5" fontFamily="monospace" fontSize="18px" fontWeight="400">
            {def.title}
          </Label>
...
function CfgInputList({ paramDef, value, setValue }) {
  ...
  return (
    <Flex direction="column" align="stretch">
      {items.length ? items : <Box style={{ paddingLeft: '.5rem' }}>No items</Box>}
      <HorizontalLine />
      <Flex direction="row">
        <CfgInput
          paramDef={paramDef.items}
